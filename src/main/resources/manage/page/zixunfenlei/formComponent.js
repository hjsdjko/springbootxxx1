Vue.component('zixunfenlei-form', {
template:`
<template>
    <div>
        <el-dialog
            ...
            <el-form class="formModel_form" ref="formRef" :model="form" :rules="rules" >
                <el-form-item  label="资讯分类" prop="zixunfenlei" class="input-item">
                    <el-input v-model="form.zixunfenlei"
                       placeholder="资讯分类"
                       type="text"
                       :readonly="!isAdd||disabledForm.zixunfenlei?true:false" ></el-input>
                </el-form-item>
            </el-form>
...
methods: {
    ...
    getInfo(){
        http.get(`zixunfenlei/info/${this.id}`).then(res=>{
    ...
    save(){
        ...
        this.$refs.formRef.validate((valid)=>{
            ...
            if(crossUserId&&crossRefId){
                ...
                http.get('zixunfenlei/page',{
                    params:params
                }).then(res=>{
                    if(res.data.data.total>=crossOptNum){
                        return this.$message.error(this.crossTips)
                    }else{
                        http.post(`zixunfenlei/${!this.form.id ? "save" : "update"}`,this.form).then(res=>{
                            if(this.type == 'cross'){
                                //修改跨表数据
